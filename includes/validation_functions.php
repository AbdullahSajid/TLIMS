...
function validate_max_lengths_for_file_names($fields_with_max_lengths) {
    global $errors;
    // Expects an assoc. array
    foreach($fields_with_max_lengths as $field => $max) {
        $value = trim($_FILES[$field]['name']);
        if (!has_max_length($value, $max)) {
            $errors[$field] = fieldname_as_text($field) . " name is too large";
        }
    }
}
