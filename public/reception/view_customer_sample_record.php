...
//                 display query errors or query success if any
    ...
    <!-- #submit_modal_confirmation start -->
    <div class="modal fade" id="submit_modal_confirmation">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×
                    </button>
                    <h4 class="modal-title">Confirm Sample Test</h4>
                </div>
                <div class="modal-body">
                    <div class="alert alert-warning m-b-0">
                        <h4>Do you want to confirm this sample submission?</h4>
                        <p>Warning!! Confirming sample submission will transfer all the sample tests information to the lab managers. Click Yes to confirm and No to cancel.</p>
                    </div>
                </div>
                <div class="modal-footer">
                    <a href="javascript:;" class="btn btn-sm btn-white"
                       data-dismiss="modal">Close</a>
                    <a href="operations/submit_customer_sample_record.php?customer_id=<?php if(isset($customer)){ echo urlencode($customer["customer_id"]);} else {echo "";} ?>" class="btn btn-sm btn-info">Confirm</a>
                </div>
            </div>
        </div>
    </div>
      <!-- #submit_modal_confirmation end-->
         ...
         <!-- begin panel -->
                <div class="panel panel-inverse" data-sortable-id="ui-typography-14">
                    ...
                    <div class="panel-toolbar text-center">
                        <?php if($customer["status"]=='submitted'){ ?>
                            <a class="btn btn-info btn-sm" href="#submit_modal_confirmation" data-toggle="modal"><i class="fa fa-check-circle"></i> Confirm</a>
                            <?php } ?>
                            <?php if($customer["status"]=='submitted'){ ?>
                            ...
                            <?php if($customer['status']=='finished'){ ?>
                             ...
                             <a class="btn btn-warning btn-sm" href="print_sample_report.php?customer_id=<?php echo $customer['customer_id']; ?>&lab=<?php echo $customer['lab']; ?>" target="_blank"><i class="fa fa-download"></i> Download Report</a>           
                            <?php } ?>
